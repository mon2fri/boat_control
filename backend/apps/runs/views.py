...
class ExecuteComparisonView(APIView):  # type: ignore[misc]
    def post(self, request: Request) -> Response:
        ...
        # Validate aggregation_columns: each must be a unique member of comparison_columns
        if aggregation_columns:
            comp_set = set(comparison_columns)
            dupes = [c for c in aggregation_columns if aggregation_columns.count(c) > 1]
            if dupes:
                return Response(
                    {"error": f"Duplicate aggregation columns: {', '.join(set(dupes))}"},
                    status=400,
                )
            invalid_gc = [c for c in aggregation_columns if c not in comp_set]
            if invalid_gc:
                return Response(
                    {"error": f"Invalid aggregation columns: {', '.join(invalid_gc)}"},
                    status=400,
                )
